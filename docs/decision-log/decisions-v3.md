...
| 2026-08-18 | U-9, D-55, `rabbitmq-schemas.md` §5.3 | Completion is decided from the `analysis_submissions` rows; the cached SIM group set is an optimisation, cleared when the batch closes | U-9 and §5.3 read as though the cached counter *is* the completion mechanism, and D2/D3's board note promised D6 would gate on it. Writing it that way makes a cache miss fatal in the wrong direction: the cache is the database store with a TTL and no durability guarantee anyone relies on, so an api restart, an eviction or a `cache:clear` would leave a batch whose every result had landed sitting at `processing` until D-82's sweeper called it a timeout — turning complete results into a reported failure. The rows already carry the whole answer, because a language group whose message never arrived leaves its submissions at `in_queue`; there is nothing the set knows that they do not. It stays because it is cheap and answers "is the SIM phase done" without scanning, and D6 forgets the key on close so it cannot outlive its subject. Applied in D6 |
| 2026-08-18 | U-8, openapi §WebSocket Channels | `analysis.progress` fires once per **newly completed submission** — when `completed_count` rises — not once per result message | openapi declares the payload and the audience but says nothing about frequency, and the two obvious readings differ by an order of magnitude: one SIM message closes an entire language group at once, so per-message would under-report badly, while per-submission is bounded by the batch size. Milestones (every 10%) were rejected because a section of eight submissions would show no progress at all until it finished, which is precisely the case where an instructor is watching. The frame carries absolute counts rather than a delta, so a client that misses one is corrected by the next. A batch is semester-wide (D-48), so one event fans out to every section in scope rather than one dispatch per section. Applied in D6 |
| 2026-08-18 | D-82, D-59 | The timeout sweeper closes batches through the same conditional update D6 uses, and leaves per-submission rows untouched | D-82 says to mark stale batches `timeout`; it does not say what happens to a batch that finishes between the sweeper's query and its write, nor to the submissions inside one that times out. Both matter. Going through `AnalysisCompletionService::close()` — whose `WHERE status = 'processing'` is the guard — means a batch that completed a moment earlier keeps its completion instead of being overwritten with a timeout it never had. And the rows are left alone because D-59 requires it: a batch that timed out with eighty of a hundred answers must still show those eighty, so a submission's status stays independent of its batch's. The timeout is announced with the existing `analysis.completed` event, whose `status` field carries the ending — no second event, no second channel. Applied in D7 |

| 2026-08-18 | D-05, D-06, openapi `SimilarityResultDetail` | Cross-section redaction is **field-level, per side, per viewer**: the row stays visible to every instructor teaching in the semester, and each `*_source_code` is shown only to an instructor of that side's section (or an Org Admin of the batch's organization). An instructor who teaches neither side sees both as null; the highlight regions follow their own side | D-05 defines two tiers and D-06 spells the cross-section one as "other student name + % + own student's code" — which silently assumes the reader owns one side. They do not always: a batch is semester-wide (D-48) and `AnalysisProblemPolicy::view` therefore admits every instructor teaching in that semester, so an instructor of L03 can open a pair between L01 and L02 and D-06 gives them nothing to own. Two readings were possible and the narrower one was rejected deliberately: filtering the *rows* would hide cross-section plagiarism from the very colleague best placed to recognise a shared source, and the openapi list schema carries both students and the score with no redaction marked on either. So the row stays and the code goes — which is exactly the balance D-05 was drawn for, extended to a viewer it did not describe. The regions travel with their side because a highlight without its code points into something the reader may not see. `?include=source_code` is explicitly not a way around any of this, as openapi already states; the include selects a field, never a permission. The whole rule lives in `AnalysisVisibilityService` so G3 has one seam to audit, and `SimilarityVisibilityTest` is its specification. Applied in D8 |
| 2026-08-18 | D-62, openapi `getAnalysisOverview` | `flagged` is computed per request from the semester's own `similarity_threshold`/`ai_detection_threshold`, and the semester overview stays Org-Admin-only | Two small holes on the same surface. The result schemas carry a boolean `flagged` without saying against what — D-62 puts the thresholds on `semesters` and they were already columns with model casts, so reading them per request (rather than freezing a value at ingest) means moving a course's bar re-colours its existing results, which is what a configurable threshold is for. And the overview is marked "(Org Admin)" in its summary only; that is now enforced, because it aggregates flagged counts across every section — precisely the cross-section picture D-05/D-06 keep from an individual instructor, so admitting instructors would undo the redaction one endpoint over. Counting is over `is_latest` batches only: a re-run replaces its predecessor (D-53), so counting every batch would inflate the totals by the number of re-runs. Applied in D8 |
| 2026-08-19 | roadmap E2, `result.analysis.v1` `a_regions`/`b_regions`, `rabbitmq-schemas.md` §4.2.1 | SIM drives Dolos through a bundled Node reporter over `@dodona/dolos-lib`, not the `dolos` CLI; regions are emitted with **1-based** rows and columns | E2 specified "CLI invocation per language, CSV/JSON output parsing → ordered pairs + regions", and the first half of that cannot produce the second. Dolos's CSV export is `pairs.csv` + `files.csv` + `metadata.csv` + `kgrams.csv`, and `pairs.csv` carries `similarity`, `totalOverlap`, `longestFragment`, `leftCovered`, `rightCovered` — no fragment coordinates anywhere; the web report recomputes them in the browser. But `a_regions`/`b_regions` *are* those coordinates, and without them F9's linked-scroll viewer has similarity scores and nothing to highlight. The library exposes exactly what the schema asks for — `pair.buildFragments()` yields `leftSelection`/`rightSelection` with `startRow/startCol/endRow/endCol` — so `reporter/report.mjs` (~90 lines, pinned `@dodona/dolos-lib`) drives it and emits one JSON document, and `internal/dolos` parses that. Two details settled with it. Dolos counts rows and columns from **0** while the viewer counts from **1**, so SIM converts on the way out: the offset then lives in one place with a test on it instead of in every consumer, and `rabbitmq-schemas.md` §4.2.1 now states the base rather than leaving it to be discovered. And `report.allPairs()` returns *every* combination including those with nothing in common — for a 150-submission group that is roughly eleven thousand rows of 0.0 — so the reporter drops pairs with zero overlap, which is what "empty array if no matches found" already meant. Applied in E2 |
| 2026-08-19 | roadmap E5, `AID_MODEL_NAME` in compose + `docker-compose-architecture.md` §env | AID's scorer is pluggable — a fine-tuned classification head when the checkpoint has one, masked-LM predictability otherwise — and the default checkpoint becomes **`microsoft/codebert-base-mlm`** | E5 said "CodeBERT (`microsoft/codebert-base`) … inference → probability", and that checkpoint cannot produce one. It is an encoder: it carries no AI-detection head, so `AutoModelForSequenceClassification` on it yields a **randomly initialised** classifier, and — measured, not assumed — it carries no trained MLM head either, so loading it as a masked LM leaves `lm_head.*` random too. Scored that way the two fixtures came out 1.4e-13 and 2.3e-13: noise with a decimal point, which api would store and an instructor would read as a finding. `microsoft/codebert-base-mlm` is Microsoft's release of the same model **with** the MLM head trained, and it scores the same fixtures at 1.46 and 1.32 nats per token — sane, and in the direction the hypothesis predicts, since generated code is more predictable to a code LM than hand-written code. So AID scores predictability by default (mean masked-token negative log-likelihood, strided so a window costs `mask_stride` forward passes rather than 510, mapped to 0–1 by a logistic) and switches to a classification head automatically when `MODEL_NAME` names a `*ForSequenceClassification` checkpoint — the upgrade path to real detection quality, with no code change. Two safeguards came out of the same finding: `_default_loader` **refuses to start** when `output_loading_info` reports missing weights, because transformers fills a missing head with random values and only warns; and the calibration constants are recorded as unfitted placeholders (there is no labelled corpus here to fit them to) that move the threshold rather than the ordering. Applied in E5 |
| 2026-08-19 | U-8, `routes/channels.php`, `config/broadcasting.php` (new) | The default broadcaster falls back to **reverb**, never `null`, and the api container selects it explicitly in compose | E9 ran the channel matrix against the deployed stack and a student authorised another student's `private-submission` channel — 200, empty body, for every channel it was asked about. The cause is that `services/api` had **no `config/broadcasting.php` at all**, so the framework default applied, and that default is `null`. The null broadcaster is not a quiet no-op: `NullBroadcaster::auth()` returns without checking anything, so `POST /broadcasting/auth` authorises everyone for everything — U-8's whole matrix, bypassed by an unset environment variable. The suite could not see it, and for a reason already on the record: C7 pins the test driver to `reverb` precisely because "the null broadcaster skips channel authorisation entirely, so the U-8 matrix would have passed against nothing" — which is exactly what the deployment was doing. Two fixes, both needed. The config file now exists with `env('BROADCAST_CONNECTION', 'reverb')`, so a missing variable costs a connection error (loud) instead of universal authorisation (silent); and compose sets `BROADCAST_CONNECTION` on api, reverb and the two consumer processes, since a broadcaster that is never selected also never broadcasts — the analysis and execution events had no transport in a deployment either. `BroadcastingConfigTest` reads the config file directly, because the test environment sets the variable and only the file shows the fallback. Applied in E9 |
| 2026-08-20 | D-82, D-59, v3 §7 (2026-08-14, `execution_status` by count) | A **submission** left unfinished is swept to `execution_status = 'timeout'` the way D-82 sweeps a batch: `submissions:sweep-timeouts` every five minutes, 60 minutes to the verdict, and the student is told over `execution.completed` | D-82 gives an analysis batch a deadline and nothing gives a submission one, so both unfinished states were terminal in practice. A grading job whose three retries are spent dead-letters and leaves its row at `processing`; a publish the broker refuses is swallowed by C3 -- deliberately, the submission is already durable -- and leaves it at `in_queue`, with re-publishing assigned to "C7/D7" and implemented by neither. Twenty-three rows were sitting in the first state on the development stack, one per message in `code-executor.dlq`. The cost is not the row, it is the student: F5's verdict strip and F6's "submit -> strip fills live" both wait on `execution.completed`, which never arrives, so the UI M4 is about to build would spin for ever with no error state to fall back to. No new vocabulary was needed -- the 2026-08-14 amendment already defines submission-level `timeout` as "the grading run did not finish", which is precisely this, and `repository.ExecutionTimeout` already existed in CES with `MarkFailed` able to write it and no production caller that ever did. **Sixty minutes, not D-82's thirty**: a submission may hold 50 test cases (D-45) each with a time limit of up to 30s (openapi), so 25 minutes of legitimate execution is reachable before Judge0's own queueing, and a sweeper that contradicts a run still in progress is worse than one that is late. The update is conditional on the row still being unfinished, which is D7's conditional close in another table: a run that finishes between the query and the write keeps the verdict it earned. Counters are left as they are, so a run that graded two of five still shows those two (D-59's spirit). Chosen over having CES write the status before it dead-letters -- that cannot cover a CES that died, which is the case the sweeper exists for. Applied in the pre-M4 cleanup |
| 2026-08-20 | roadmap B12, openapi `uploadAvatar`/`uploadOrgAvatar` | Avatars are capped at **2 MiB** and restricted to **jpeg/png/webp**, stored under `avatars/{users\|organizations}/{id}/{random}.{ext}` by a service of their own | B12's Verify column asks for "avatar mime/size validation" and neither openapi nor any D-decision gives the numbers -- openapi says `format: binary` and stops. Something had to be chosen, because the api *streams* these objects rather than pre-signing them (v3 §7, 2026-08-14), so an unbounded upload is a slow response for every viewer of that person's name afterwards, paid by the api process rather than by MinIO. 2 MiB is generous for the 512px avatar the UI renders and still cheap to stream; the three formats are the ones every target browser decodes, and accepting more would mean either transcoding or serving something a browser will not draw. The key carries a random component so a replaced avatar cannot collide with a cached copy of the one it replaced, and the old object is deleted only **after** the row points at the new one -- the failure that leaves an orphan is better than the one that leaves a row pointing at nothing. Kept in `AvatarStorageService` rather than added to `SubmissionStorageService`: that service's key must stay byte-identical to `shared/go/storage.SubmissionPath` because the Go workers read the same objects back, and nothing outside api ever reads an avatar. Applied in B12 |
