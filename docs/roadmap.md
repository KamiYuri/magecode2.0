...
| A7 | Move worker modules to `services/{code-executor,plagiarism-checker,vuln-scanner}/`, port `services/ai-detector` skeleton, fix `go.work`, port Dockerfiles (multi-stage Alpine, D-79a) | Med | P0 | Building scaffolds wired to shared packages | `go build ./...` all modules; `docker build` each image < 30MB for Go (**holds for CES alone from E8**: SIM carries Node + tree-sitter and VUL the CodeQL bundle); compose `--profile analysis config` validates |
...
| E8 | Worker images + compose `analysis` profile wiring, healthchecks (D-72 — workers had no HTTP surface, so `shared/go/health` and `src/health.py` add one), Loki logging labels, model/codeql cache volumes, **and the three api-image processes nothing was running** (`amqp:consume-execution`, `amqp:consume-analysis`, `schedule:work`) | Med | P1 | Deployable worker images | `docker compose --profile analysis up`: all healthy; logs visible in Loki with `service` label |
